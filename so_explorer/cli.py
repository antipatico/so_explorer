...
def symbols_summary(session, limit=1000):
    ...
    rc = (
        session.query(Symbol.name, Symbol.s_type, func.count(Symbol.s_type))
        .group_by(Symbol.name, Symbol.s_type)
...
def symbols_search(session, search, limit=1000):
    ...
    rc = (
        session.query(Symbol.name, Symbol.s_type, func.count(Symbol.s_type))
        .filter(Symbol.name.like(search))
        .group_by(Symbol.name, Symbol.s_type)
        .order_by(Symbol.name)
        .limit(limit)
    )
...
def process_sofile(filename):
    ...
    if result is not None:
        print(
            "WARNING: file %s is already in db as %s"
            % (os.path.basename(filename), result.filename)
        )
        return
...
def main():
    parser = argparse.ArgumentParser(
        prog="so_explorer", description="Easily browse symbols from modules"
    )
    parser.add_argument(
        "-d",
        "--database",
    ...
    build_parser.add_argument(
        "-r", "--recursive", help="scan ELF files recursively", action="store_true"
    )
    build_parser.add_argument(
        "files", type=str, nargs="+", help="files or directories to add to database"
    )
